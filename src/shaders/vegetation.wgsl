// Vegetation: instanced, TEXTURED 3D plant models (see src/models.rs). Each
// archetype's base mesh is uploaded once and drawn many times with a per-instance
// model matrix + colour tint, so a chunk stores ~80 bytes per plant. Every vertex
// carries a UV and a layer into a shared texture_2d_array; foliage/spine materials
// are alpha-cutout cards, so the fragment alpha-tests. Lit and fogged like the
// terrain (but no water — plants are never sea).
...
const ALPHA_CUTOFF: f32 = 0.45;    // discard below this texture alpha (models authored MASK @ 0.4–0.5)
...
// Shared vegetation texture array (every archetype material's baseColorTexture) +
// its repeat sampler (the tiling trunk/bark UVs need REPEAT addressing).
@group(1) @binding(0) var veg_tex: texture_2d_array<f32>;
@group(1) @binding(1) var veg_samp: sampler;
...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
    // Albedo from the model texture; alpha-cut the foliage/spine cards.
    let tex = textureSample(veg_tex, veg_samp, in.uv, in.layer);
    if (tex.a < ALPHA_CUTOFF) {
        discard;
    }
    let albedo = tex.rgb * in.tint;

    // Foliage/leaf cards are double-sided (culling is off), so flip the normal to
    // face the viewer before lighting — otherwise back-facing leaves read black.
    ...
    var col = albedo * (amb + wrap * (1.0 - amb) + sky_fill + trans);
